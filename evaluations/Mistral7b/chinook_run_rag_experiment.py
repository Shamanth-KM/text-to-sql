...
with open("/content/drive/MyDrive/text2SQL/knowledge/chinook_knowledge_base.txt", 'r', encoding='utf-8') as file:
    content = file.read()
    rag_kb = ast.literal_eval(content)
...
def chinook_run_rag_experiment(tokenizer, model):
    # Load model
    ...
    for idx, example in enumerate(prompts):
        ...
        if (idx + 1) % SAVE_INTERVAL == 0:
            partial_path = "/content/drive/MyDrive/text2SQL/results/partial_mistral_rag_chinook_exp_res.csv"
            pd.DataFrame(results).to_csv(partial_path, index=False)
            print(f"Saved partial results at {idx+1} prompts to {partial_path}")

    # Save results to CSV
    os.makedirs("results", exist_ok=True)
    output_path = "/content/drive/MyDrive/text2SQL/results/mistral_rag_chinook_exp_res.csv"
...
if __name__ == "__main__":
    tokenizer, model = load_model_and_tokenizer()
    chinook_run_rag_experiment(tokenizer,model)