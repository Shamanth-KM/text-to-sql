...
def spider_run_experiment(tokenizer, model):
    # Load model
    ...
    for idx, example in enumerate(prompts):
        ...
        if (idx + 1) % SAVE_INTERVAL == 0:
            partial_path = "/content/drive/MyDrive/text2SQL/results/partial_mistral_baseline_spider_exp_res.csv"
            pd.DataFrame(results).to_csv(partial_path, index=False)
            print(f"Saved partial results at {idx+1} prompts to {partial_path}")

    # Save results to CSV
    os.makedirs("results", exist_ok=True)
    output_path = "/content/drive/MyDrive/text2SQL/results/mistral_baseline_spider_exp_res.csv"
...
if __name__ == "__main__":
    tokenizer, model = load_model_and_tokenizer()
    spider_run_experiment(tokenizer,model)